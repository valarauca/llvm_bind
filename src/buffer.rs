...
impl Buffer {
  ...
  /// it to the LLVM.
  pub fn with_capacity<S>(size: usize, name: S)
  -> Self
  where S: Into<Vec<u8>> {
  ...
  /// from another buffer.
  pub fn copy_from<S>(buf: &[u8], name: S)
  -> Self
  where S: Into<Vec<u8>> {
    let name = CString::new(name).expect(NULLPTR);
    unsafe{
  ...
  /// an LLVM Module
  pub fn parse_ir(mut self)
  -> Result<Module,(Buffer,CString)> {
    use std::mem;
    
    let mut modu = Module::new(self.get_name().to_bytes());
    let mut v = Vec::<Buffers>::with_capacity(0);
    mem::swap(&mut v, &mut self.buffers);
    unsafe {
      let mut err: *mut c_char = mem::zeroed();
      let flag = LLVMParseBitcode(
        self.data,
        modu.raw_module(),
        &mut err
      );
  ...
  pub fn from_owned<S>(buf: Vec<u8>, name: S)
  -> Self 
  where S: Into<Vec<u8>> {
    let name = CString::new(name).expect(NULLPTR);
    unsafe{
      ...
      let llvm = LLVMCreateMemoryBufferWithMemoryRange(
                    ptr,
                    len,
                    n_ptr,
                    0
  ...
  /// From raw
  ///
  /// unsafely buids this item from it's raw components
  pub unsafe fn from_raw(x: LLVMMemoryBufferRef, buf: Vec<Buffers>)
  -> Self {
    Buffer {
      data: x,
      buffers: buf
