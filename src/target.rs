...
impl Target {
    ...
    /// Can the LLVM library target this backend(?) I think(?)
    ...
    ///
    /// Generally this method is used internal to libraries
...
impl Default for CodeGenOptLevel {
    /// Returns the Enum that is labeled as `default` by the llvm source so
    /// I'm assuming that is a good default.
    #[inline(always)]
    fn default() -> CodeGenOptLevel {
...
///
/// I have no clue what these do, and the LLVM has next to no doc.
#[derive(Copy,Clone,Debug)]
pub enum RelocMode {
    Default,
    Static,
...
/// Code Model
///
/// This gives some hint on how the LLVM should optimize code. Is it a JIT, a GPU kernel,
/// or is it optimizing for low instructions... or fast many instructions.
#[derive(Copy,Clone,Debug)]
pub enum CodeModel {
    Default,
    JIT,
...
/// Target Machine
/// 
/// Describe the physical machine that is being compiled too. 
...
impl Default for TargetMachine {
    ...
    fn default() -> TargetMachine {
        let cpu = CString::new("generic").unwrap();
        let features = CString::new("").unwrap();
        let t_ptr = Target::default();
        let (tar,buffs) = unsafe{
            t_ptr.split()
        };
...
impl BuildTargetMachine {
    ...
    pub fn set_cpu<S: AsRef<str>>(&mut self, cpu: S) -> &mut Self {
        let cpu = cpu.as_ref();
    ...
    pub fn set_target_triple<S: AsRef<str>>(&mut self, triple: S) -> &mut Self {
    ...
    /// Complete the `TargetMachine` builder pattern    
